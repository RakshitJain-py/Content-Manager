...
export interface ToggleRowProps {
  ...
  defaultOn?: boolean;
  /** Optional callback fired with the new state. */
  onChange?: (on: boolean) => void;
}

/** Full-width switch row used in the configuration panel. */
export function ToggleRow({ label, defaultOn = false, onChange }: ToggleRowProps) {
  ...
  return (
    ...
    >
      {label}
      <span className={cn("relative h-5 w-9 rounded-full transition-colors", on ? "ig-gradient" : "bg-neutral-700")}>
        <span
          className={cn(
